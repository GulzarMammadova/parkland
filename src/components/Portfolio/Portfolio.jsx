...
const DEFAULT_PROJECTS = [
  { title:"Modern Zen Garden", category:"Residential", images:[
  ...
  { title:"Estate Landscape", category:"Luxury Residential", images:[
  ...
  { title:"Contemporary Garden", category:"Urban Design", images:[
    "https://images.unsplash.com/photo-1553194588-ecc5e217ebf0?auto=format&w=1200&q=80",
  ...
  { title:"Backyard Oasis", category:"Residential Retreat", images:[
...
export function Portfolio(){
  ...
  const heading = lang==="EN" ? "Featured Projects" : "Seçilmiş Layihələr";
  const sub = lang==="EN"
    ? "Discover our portfolio of transformative landscape designs that blend luxury with nature’s inherent beauty."
    : "Lüksü təbiətin doğma gözəlliyi ilə birləşdirən landşaft layihələrimizi kəşf edin.";

  const projects = DEFAULT_PROJECTS; // <-- или подставь свой массив
  ...
  return (
    <section id="portfolio" className="portfolio section" ref={ref}>
      <div className="container">
        <motion.div initial={{opacity:0,y:30}} animate={inView?{opacity:1,y:0}:{}} transition={{duration:.8}} className="portfolio__header">
          <h2 className="portfolio__title">{heading}</h2>
          <p className="portfolio__sub">{sub}</p>
        </motion.div>
        ...
        {len>1 && (
          <div className="portfolio__dots">
            {projects.map((_,i)=>{
              const active = i===current || i===(current+1)%len;
              return (
                <button key={i} className={`portfolio__dot ${active?"is-active":""}`} onClick={()=>setCurrent(i)} aria-label={`Go to project ${i+1}`}/>
              );
            })}
          </div>
        )}
...
function ProjectCard({ project, inView }){
  ...
  const imgs = Array.isArray(project.images)&&project.images.length>0 ? project.images : ["https://images.unsplash.com/photo-1612143241883-35889d1d65ab?auto=format&w=1200&q=80"];
  ...
  return (
    <motion.div initial={{opacity:0,y:30}} animate={inView?{opacity:1,y:0}:{}} transition={{duration:.6}} className="pCard">
      <div className="pCard__imageWrap">
        {imgs.map((src,i)=>(
          <div key={i} className={`pCard__imageStage ${i===idx ? "is-active":""}`}>
            <ImageWithFallback className="pCard__image" src={src} alt={`${project.title} ${i+1}`}/>
          </div>
        ))}
        <div className="pCard__overlay"></div>
      </div>
